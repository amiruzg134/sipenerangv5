    ...
    <script type="text/javascript">
        ...
        $('#checkout').click(function () {
            ...
            for (let i = 1; i <= total_anggota; i++) {
                let elemen = document.getElementById("anggota_status" + i);

                if (elemen) {
                    let status = elemen.textContent?.trim();
                    if (!status) {
                        adaKosong = true;
            ...
            if (adaKosong) {
                alert("Ada anggota yang masih belum di tambahkan. Silakan cek ulang.");
                return false;
            } else if (adaInvalid) {
                alert("Anggota anda ada yang memiliki status INVALID, silakan cek ulang.");
                return false;
            }
                    ...
                    $.ajax({
                        ...
                        success: function (response_tiket){
                            ...
                            if (json.data.can_booking_before_day > 0){
                                informasi_tanggal.textContent = "Pemesanan dapat dilakukan maksimal H-"+json.data.can_booking_before_day;
