...
try{
    ...
    if (
        ($current_time >= '23:00' && $current_time <= '23:59') || ($current_time >= '00:00' && $current_time < '04:00')
    ){
        $respon = [
            "error" => true,
            'message' => "Transaksi tidak dapat dilakukan antara pukul 23:00 sampai 04:00 dengan metode pembayaran VA.",
            "data" => null
        ];
    }

    if ($tb_gunung_id == 1 && $start_date == $end_date) {
        $respon = [
            "error" => true,
    ...
    $sqlKuotaUpdate = "UPDATE tiket_kuota SET stock_available = stock_available - $total_anggota WHERE tb_gunung_id='$tb_gunung_id'
                                                                   AND tb_pos_pendaki_id='$tb_pos_pendakian_id' AND date='$start_date' 
    ...
    $code_pos_pendakian = $mountainGate['code'] ?? date('y');
