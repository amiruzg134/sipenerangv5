...
require_once ('../config/ektensi.php');
    ...
    $format_end_date    = strtotime($start_date);

    $set_start_date  = date('Y-m-d', strtotime($start_date));

    $sqlCek = mysqli_query($conn, "SELECT * FROM tiket_kuota 
    WHERE tb_gunung_id='$gunung_id' 
      AND tb_pos_pendaki_id='$pos_id' 
      AND date='$set_start_date' 
      AND is_active=1");
    ...
    $is_ready = false;
    if (!empty($kuotaCek)) {
    $stock_available = $kuotaCek['stock_available'];
    $sisa_kuota = $stock_available - $total_anggota;
