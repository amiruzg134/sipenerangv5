    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#proses', function() {
            ...
            if (file) {
                ...
                const emergencyName = $('#emergency_name_one').val()?.trim();
                const emergencyPhoneCode = $('#emergency_phone_code_id_one').val()?.trim();
                const emergencyPhone = $('#emergency_phone_one').val()?.trim();
                const emergencyRelationship = $('#emergency_relationship_one').val()?.trim();

                if (!emergencyName || !emergencyPhoneCode || !emergencyPhone || !emergencyRelationship) {
                    alert('Semua data kontak darurat wajib diisi.');
                    return false;
                }
                ...
                formData.append('emergency_name_one', emergencyName);
                formData.append('emergency_phone_code_id_one', emergencyPhoneCode);
                formData.append('emergency_phone_one', emergencyPhone);
                formData.append('emergency_relationship_one', emergencyRelationship);
                $.ajax({
                    type    : "POST",
