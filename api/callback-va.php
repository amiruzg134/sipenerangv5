...
try{
    ...
    $virtual_account = $entityBody->virtual_account;
    $amount          = $entityBody->amount;
    $reference       = $entityBody->reference;
    ...
    $trx = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM tb_pendakian WHERE payment_number='$virtual_account'"));
    $response = $client->post($env_base_url.'callback-va', [
            'form_params' => [
    ...
    if($res['error']){
        ...
        $sql = mysqli_query($conn, "UPDATE tb_pendakian SET tgl_bayar = '$tanggal',
                sts_bayar='paid', pd_status='disetujui' WHERE pd_id='$trx_id'");
        $respon = [
            "error"   => false,
            "message" => $res['message'],
            "data"    => null,
