...
if($_GET['action'] == "grafik_perpos"){
    try {
        if (isset($_POST['tahun_select'])) {
            $year = Carbon::now()->format('Y');
        }else{
            $year = $_POST['tahun_select'];
        }
        $posSql = mysqli_query($conn, "SELECT pp_id, pp_nama FROM tb_pos_pendakian WHERE is_active=1");
        $pos    = null;
