...
?>
    ...
    $pendaki        = mysqli_query($conn, "SELECT * FROM tb_pendakian WHERE  is_region_new=1 AND tb_gunung_id='$gunung_select' AND pd_pos_pendakian='$pos_select' AND date(tgl_bayar) BETWEEN '$set_start_date' AND '$set_end_date' AND sts_bayar = 'paid'");
    $pendaki_turun  = mysqli_query($conn, "SELECT * FROM tb_pendakian WHERE is_region_new=1 AND tb_gunung_id='$gunung_select' AND pd_pos_turun='$pos_select' AND date(pd_tgl_turun) BETWEEN '$set_start_date' AND '$set_end_date' AND denda > 0 ORDER BY pd_tgl_naik ASC");
    $total_denda = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(denda) AS denda FROM tb_pendakian WHERE is_region_new=1 AND tb_gunung_id='$gunung_select' AND pd_pos_turun = '$pos_select' AND date(pd_tgl_turun) BETWEEN '$set_start_date' AND '$set_end_date' AND denda > 0"));
    ?>
    ...
    <table>
        ...
        <tr>
            <td colspan="4"><h4  style="text-transform: none;">Laporan Denda <?php echo $gunung['nama'].'('.$pos['pp_nama'].')'; ?></h4></td>
        </tr>
        <tr>
                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content" style="border: none; padding: 15px 0 20px 0">
                        ...
                        <table class="table table-bordered" style="width: 100%">
                            ...
                            $jumwni = 0;
                            $jumwna = 0;
                            while ($row = mysqli_fetch_array($pendaki_turun)) { ?>
                                <tr style="font-size: 10px">
                                    <td><?php echo $no++; ?></td>
                                    <td><?php echo $row['pd_nomor']; ?></td>
                                    <td><?php echo $row['pd_nama_ketua']; ?></td>
                                    <td style="text-align: center"><?php echo Carbon::parse($row['tgl_naik'])->format('d-n-Y').'<small> (rencana)</small><br>'.Carbon::parse($row['pd_tgl_naik'])->format('d-n-Y'); ?></td>
                                    <td style="text-align: center"><?php echo Carbon::parse($row['tgl_turun'])->format('d-n-Y').'<small> (rencana)</small><br>'.Carbon::parse($row['pd_tgl_turun'])->format('d-n-Y'); ?></td>
                                    <?php
                                    ...
                                    <td style="text-align: center;"><?php echo number_format($row['denda'],0,",",".") ?></td>
                                </tr>
                            <?php }
                            ?>
                            <tr>
                                ...
                                <td style="text-align: center;"><strong><?php echo 'Rp. '.number_format($total_denda['denda'],0,",",".") ?></strong></td>
                            </tr>
