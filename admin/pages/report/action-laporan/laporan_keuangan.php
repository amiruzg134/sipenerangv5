...
?>
    ...
    $pendaki = mysqli_query($conn, "SELECT * FROM tb_pendakian WHERE is_region_new=1 AND tb_gunung_id= '$gunung_select' AND pd_pos_pendakian='$pos_select' AND date(tgl_bayar) BETWEEN '$set_start_date' AND '$set_end_date' AND sts_bayar ='paid' ORDER BY tgl_bayar ASC");


    $sql3 = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(biaya) AS biaya FROM tb_pendakian WHERE is_region_new=1 AND tb_gunung_id= '$gunung_select' AND pd_pos_pendakian='$pos_select' AND date(tgl_bayar) BETWEEN '$set_start_date' AND '$set_end_date' AND sts_bayar = 'paid' "));
    ...
    <table>
        ...
        <tr>
            <td colspan="4"><h4  style="text-transform: none;">Laporan Keuangan <?php echo $gunung['nama'].'('.$pos['pp_nama'].')'; ?></h4></td>
        </tr>
        <tr>
                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content" style="border: none; padding: 15px 0 20px 0">
                        ...
                        <table class="table table-bordered" style="width: 100%">
                            ...
                            $jumwni = 0;
                            $jumwna = 0;
                            while ($row = mysqli_fetch_array($pendaki)) { ?>
                                <tr style="font-size: 10px">
                                    <td><?php echo $no++; ?></td>
                                    <td><?php echo $row['pd_nomor']; ?></td>
                                    <td><?php echo $row['pd_nama_ketua']; ?></td>
                                    <td style="text-align: center"><?php echo Carbon::parse($row['pd_tgl_naik'])->format('d-m-Y H:i'); ?></td>
                                    <?php
                                    if ($row['pd_pos_turun'] == '') { ?>
                                        <td style="text-align: center"><?php echo Carbon::parse($row['pd_tgl_turun'])->format('d-m-Y H:i');?><small><strong>(rencana)</strong></small></td>
                                    <?php }
                                    else{ ?>
                                        <td style="text-align: center"><?php echo Carbon::parse($row['pd_tgl_turun'])->format('d-m-Y H:i'); ?></td>
                                    <?php }?>
                                    <?php
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;"><strong><?php echo 'Rp. '.number_format($sql3['biaya'],0,",",".") ?></strong></td>
                            </tr>
