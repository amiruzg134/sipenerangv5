...
if($_GET['action'] == "detail"){
    ...
    try {
        ...
        $cek_code = mysqli_query($conn, "SELECT * FROM tb_pos_pendakian WHERE code = '$code' AND pp_id != '$id'");
        if (mysqli_num_rows($cek_code) > 0) {
            $respon = [
                "error" => true,
                "message" => "Kode sudah digunakan. Harap gunakan kode yang berbeda."
            ];
            echo json_encode($respon, true);
            exit();
        }
        ...
        mysqli_query($conn, "UPDATE tb_pos_pendakian SET tb_gunung_id='$gunung_id',
                            ...
                            max_pesan='$max_pesan', can_booking_before_day='$can_booking_before_day', code='$code', is_active='$is_active' WHERE pp_id='$id'");
