...
if($_GET['action'] == "detail"){
    try {
        ...
        $arr_role = [
            "rm_jabatan"    => $rowrole != null ? $rowrole['rm_jabatan'] : 'N',
            "rm_gunung"     => $rowrole != null ? $rowrole['rm_gunung'] : 'N',
            "rm_pos"        => $rowrole != null ? $rowrole['rm_pos'] : 'N',
            "rm_pengguna"   => $rowrole != null ? $rowrole['rm_pengguna'] : 'N',
            "rm_pendaki"    => $rowrole != null ? $rowrole['rm_pendaki'] : 'N',
            "rm_laporan"    => $rowrole != null ? $rowrole['rm_laporan'] : 'N',
            "rm_acc"        => $rowrole != null ? $rowrole['rm_acc'] : 'N',
            "rm_broad"      => $rowrole != null ? $rowrole['rm_broad'] : 'N',
            "rm_kuota"      => $rowrole != null ? $rowrole['rm_kuota'] : 'N',
            "rm_verification_account"  => $rowrole != null ? $rowrole['rm_verification_account'] : 'N',
            "rm_konfigurasi"  => $rowrole != null ? $rowrole['rm_konfigurasi'] : 'N',
            "edit"          => $rowrole != null ? $rowrole['edit'] : 'N',
        ];
    ...
    try {
        ...
        $rm_jabatan     = defaultIfEmptyRole($_POST['data_checked_jabatan']);
        $rm_gunung      = defaultIfEmptyRole($_POST['data_checked_gunung']);
        $rm_pos         = defaultIfEmptyRole($_POST['data_checked_pos']);
        $rm_pengguna    = defaultIfEmptyRole($_POST['data_checked_pengguna']);
        $rm_kuota       = defaultIfEmptyRole($_POST['data_checked_kuota']);
        $rm_pendaki     = defaultIfEmptyRole($_POST['data_checked_pendaki']);
        $rm_laporan     = defaultIfEmptyRole($_POST['data_checked_laporan']);
        $rm_verifikator = defaultIfEmptyRole($_POST['data_checked_verifikator']);
        $rm_broadcast   = defaultIfEmptyRole($_POST['data_checked_broadcast']);
        $rm_verification_account = defaultIfEmptyRole($_POST['data_checked_verifikasi_akun']);
        $rm_konfigurasi = defaultIfEmptyRole($_POST['data_checked_konfigurasi']);
    ...
    try {
        ...
        $cek = mysqli_query($conn, "SELECT COUNT(*) as total FROM tb_role_menu WHERE rm_user_id = '$id'");
        $row = mysqli_fetch_assoc($cek);

        if ($row['total'] > 1) {
            mysqli_query($conn, "DELETE FROM tb_role_menu WHERE rm_user_id = '$id'");
            mysqli_query($conn, "INSERT INTO tb_role_menu (
                rm_user_id, rm_jabatan, rm_gunung, rm_pos, rm_pengguna, rm_pendaki, rm_laporan,
                rm_acc, rm_broad, rm_kuota, rm_verification_account, rm_konfigurasi
            ) VALUES (
                '$id', '$rm_jabatan', '$rm_gunung', '$rm_pos', '$rm_pengguna', '$rm_pendaki', '$rm_laporan',
                '$rm_verifikator', '$rm_broadcast', '$rm_kuota', '$rm_verification_account', '$rm_konfigurasi'
            )");

        } elseif ($row['total'] == 1) {
            // Lakukan update jika hanya 1 data
